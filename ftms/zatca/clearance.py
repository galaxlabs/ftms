...
@frappe.whitelist()
def submit_to_zatca(invoice_name, doctype="Trip Invoice"):
    adapter = get_adapter(doctype)
    invoice = adapter.get_invoice(invoice_name)
    company = adapter.get_company(invoice)
    ...
    xml_string, uuid_str = generate_invoice_xml(
        invoice, company_doc, csid_settings, invoice_counter, pih, adapter=adapter
    )
    ...
    invoice_type_code, invoice_type_name = _get_invoice_type_code(adapter.get_invoice_type(invoice))
    ...
    adapter.save_zatca_data(invoice, {
        ...
        "zatca_submit_time": frappe.utils.now_datetime(),
        "invoice_xml": signed_xml,
    })

    log_transaction(
        company, doctype, invoice.name,
